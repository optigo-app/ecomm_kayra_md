...
const Lookbook = () => {
  ...
  // useEffect(() => {
  //   let LastPageNo = JSON.parse(sessionStorage.getItem('lookbookPage'));
  //   if (LastPageNo) {
  ...
  const SlideIndexHandler = (slide) => {
    // alert(slide?.activeIndex)
  ...
  useEffect(() => {
    try {
      const RetrieveSlideIndex = sessionStorage.getItem('currentSlideIndex');
  ...
  useEffect(() => {
    if (!isProdLoading && LookLastProductId) {
      const parsed = JSON?.parse(LookLastProductId)
      try {
        HandleMoveToMenu(parsed, 100);
      } catch (error) {
              ...
              {filterData?.length > 0 && (
                <div className="smr1_lookBookFilterSubDiv" >
                  <span className="smr_filter_text">
                    ...
                    <span className="clear_All_options" onClick={() => handelFilterClearAll()}>
                      {Object.values(filterChecked).filter((ele) => ele.checked)
                        ?.length > 0
                        ? "Clear All"
                        : ""}
                    ...
                    <div className="smr_lookBookImgDivMain">
                      {filteredDesignSetLstData?.length == 0 ? (
                        ...
                        filteredDesignSetLstData?.map((slide, index) => (
                          <div className="smr_designSetDiv" Product-last-location={slide?.designsetno?.split(" ")?.join("") + slide?.designsetuniqueno?.toString()} key={index}>
                            <div
                              style={{
                                display: "flex",
                  ...
                  {!isPgLoading ? (
                    <div className="smr_lookBookImgDivMain">
                      {filteredDesignSetLstData?.length == 0 ? (
                        ...
                        <>
                          {filteredDesignSetLstData?.map((slide, index) => (
                            <div Product-last-location={slide?.designsetno?.split(" ")?.join("") + slide?.designsetuniqueno?.toString()} className="smr_designSetDiv2" key={index}>
                              <div
                                style={{
                                  display: "flex",
                                  width: "30%",
                    ...
                    <div className="smr_lookbook3MainDiv">
                      {filteredDesignSetLstData?.length == 0 ? (
                        ...
                        <>
                          ...
                          >
                            {filteredDesignSetLstData?.map((slide, index) => (
                              <SwiperSlide key={slide?.designsetuniqueno || index} >
                                <div>
                                  <div className="smr1_lb3compeletethelook_cont" Product-last-location={slide?.designsetno?.split(" ")?.join("") + slide?.designsetuniqueno?.toString()}>

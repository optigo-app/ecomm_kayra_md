...
export default function Account() {
    ...
    useEffect(() => {
        const IsHaveOrderId = sessionStorage.getItem('OrderId');
        if (IsHaveOrderId && IsHaveOrderId?.trim() !== '') {
          setValue(1);
        }
      }, []);
                    ...
                    <div className='sticky-header_web_sm'>
                        ...
                        <div className='smlingAccountTabWebView_SMR yourAccount_SMR d_none_acc_smr' >
                                    <Box sx={{ display: 'flex', justifyContent: 'center', borderBottom: 1, borderColor: 'divider' }}>
                                        <Tabs value={value} onChange={handleChange} onClick={handlePageShift} aria-label="basic tabs example"  >   {/*  orientation="vertical" indicatorColor="#7d7f85" */}

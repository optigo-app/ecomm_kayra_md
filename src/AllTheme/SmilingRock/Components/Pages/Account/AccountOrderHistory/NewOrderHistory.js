...
const NewOrderHistory = () => {
  ...
  const [LastProductTrack,SetLastProductTrack] = useState('');
    ...
    try {
      ...
      if (response?.Status === "200") {
        if (response?.Data?.rd) {
          ...
          setLoaderOH(false);
          setTimeout(() => {
            const LastViewedProduct = sessionStorage.getItem("OrderId");
            SetLastProductTrack(LastViewedProduct)
            if(LastViewedProduct){
              HandleMoveToMenu(LastViewedProduct,320)
            }
          }, 1000);
          setTimeout(() => {
            sessionStorage.setItem("OrderId", "");
            SetLastProductTrack(null);
          }, 3000);
        } else {
          setLoaderOH(true);
          setOrderHistoryData([]);
  ...
  const handleMoveToDetail = (productData,e) => {
    let loginInfo = JSON.parse(sessionStorage.getItem("loginUserDetail"));
            ...
            <div style={{ width: "100%" }}>
              {orderHistoryData?.length > 0
                ? orderHistoryData?.map((e, i) => {
                    return (
                      <Card   Product-last-location={`${e?.id}`} variant="outlined" sx={{ boxShadow: "none", width: "100%", margin: "20px 0px",    boxShadow: LastProductTrack == e?.id ? "rgba(0, 0, 0, 0.35) 0px 5px 15px" : "none", border: "1px solid #cacaca", }} key={i} >
                                  ...
                                  {images?.length > 0 && images?.map((el, index) => (
                                    ...
                                    >
                                      ...
                                      <img 
                                          ...
                                          onClick={() => handleMoveToDetail(el ,e)} 
                                      />

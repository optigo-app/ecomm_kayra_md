...
import { DfineLoadingFallback, KayraLoadingFallback, MdLoadingFallback } from "./LoadingFallbacks";
...
export default function ThemeRoutes() {
  ...
  const storeInitRaw = sessionStorage.getItem("storeInit");
  ...
  const fetchWithRetry = async (url, retries = 3, delay = 1000) => {
    ...
    for (let i = 0; i <= retries; i++) {
      try {
        const response = await fetch(url, { cache: "no-store" }); // prevent stale cache
  ...
  useEffect(() => {
    const loadStoreInit = async () => {
      const path = `${storInitDataPath()}/StoreInit.json`;

      try {
        const jsonData = await fetchWithRetry(path, 2, 1000); // Max 3 tries (1 + 2 retries)

        if (!jsonData?.rd?.length) {
          console.warn("StoreInit.json is missing expected structure");
          return;
        }

        const [rd0] = storeInitRaw.rd;

        setHtmlContent(jsonData);
        setStoreInitData(rd0);
        setIsStoreInitLoaded(true);

        if (rd0.Themeno && rd0.Themeno !== currentTheme) {
          setCurrentTheme(rd0.Themeno);
        }
        ...
        setTimeout(() => {
          ...
          const milliseconds = Math.floor(duration % 10000);

          console.log(`execution time theme page: ${minutes}m ${seconds}s ${milliseconds}ms`);
        }, [])
        ...
        console.error("Failed to load StoreInit.json:", error.message);
      }
    };
  ...
  useEffect(() => {
    if (!storeInitRaw) return;

    const storeinit = JSON.parse(storeInitRaw);
...
const Themes = ({ themeNumber }) => {
  return (
    ...
    // <Suspense fallback={<MdLoadingFallback />}>
    // <Suspense fallback={<KayraLoadingFallback />}>
    // <Suspense fallback={<DfineLoadingFallback />}>
    <Suspense fallback={<KayraLoadingFallback />}>
